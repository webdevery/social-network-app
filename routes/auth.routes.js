...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res.status(201).json({ success: true, email, password, message: "Пользователь создан" });
    } catch (e) {
      res.status(500).json({ message: "Что то пошло не так, попробууйте снова" });
    }
  }
);
...
router.post(
  ...
  [
    check("email", "Некорректный Email")
      .normalizeEmail()
      .isEmail(),
    check("password", "Введитте пароль").exists()
  ],
  async (req, res) => {
    try {
      const errors = validationResult(req);
